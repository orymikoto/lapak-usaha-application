  ...
  <div class="flex flex-col items-center flex-1 my-4 gap-x-4 mx-4">
    ...
    <div class="flex rounded-full bg-neutral-400 w-[40rem] overflow-hidden">
      ...
      <a href="/admin/daftar-pendana"
        class="flex-1 hover:bg-yellow-500 text-neutral-800 font-medium font-roboto py-1 text-center cursor-pointer hover:text-white duration-200">Daftar Pendana
      </a>
      ...
      <div class="grid grid-cols-12 gap-4 my-4 w-[60rem] ">
        @foreach ($pencairan as $key => $value)
          <div class="animate-fade-in-down p-4 col-span-3 rounded-lg flex flex-col bg-white gap-2 items-center">
            ...
            <a href="/admin/pendanaan/detail/{{ $value->id_proyek_pendanaan }}" class="text-sm text-neutral-600 hover:text-yellow-500 duration-200">Lihat Detail Pendanaan</a>
            <div class="flex flex-col w-full items-center text-neutral-800">
              <button {{ $value->status_pencairan == false ? 'disabled' : '' }} onclick="showUpdateModal('{{ '/admin/pencairan/tolak/' . $value->id_pencairan_dana }}', 'Jadikan status pencairan menjadi Menunggu Persestujuan?')" class="rounded-full bg-neutral-400 font-roboto text-sm w-full font-medium text-center cursor-default py-1 {{ $value->status_pencairan == false ? 'bg-yellow-500 text-white' : 'cursor-pointer hover:bg-yellow-500 hover:text-white duration-200' }}">Menunggu Persetujuan</button>
              <img src="/icons/next-down.svg" class="w-4 h-4" alt="">
              <button {{ $value->status_pencairan == true ? 'disabled' : '' }} onclick="showUpdateModal('{{ '/admin/pencairan/setujui/' . $value->id_pencairan_dana }}', 'Jadikan status pencairan menjadi Disetujui')" class="rounded-full bg-neutral-400 font-roboto text-sm w-full font-medium text-center cursor-default py-1 {{ $value->status_pencairan == true ? 'bg-yellow-500 text-white' : 'cursor-pointer hover:bg-yellow-500 hover:text-white duration-200' }}">Berhasil Dicairkan</button>
            </div>
