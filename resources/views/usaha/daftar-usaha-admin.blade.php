  ...
  <div id="tidak_konfirmasi" class="hidden w-full h-full z-10 flex items-center justify-center absolute bg-neutral-500/60">
    <div class="p-4 bg-white rounded-md flex flex-col items-center text-neutral-700 font-roboto font-medium gap-2 text-center">
      ...
      <div class="flex gap-x-2">
        <button
          class="py-1 w-[7rem] text-center bg-red-500 text-white hover:text-red-500 hover:bg-white rounded-md hover:shadow-md hover:shadow-red-500/50"
          onclick="hideTidakKonfirmasi()">Tidak</button>
        <a id="link_tidak_konfirmasi" href="/daftar-usaha/hapus/"
          class="py-1 w-[7rem] text-center bg-emerald-400 text-white hover:text-emerald-400 hover:bg-white rounded-md hover:shadow-md hover:shadow-emerald-400/50">Ya</a>
      </div>
    </div>
  </div>
    ...
    @foreach ($daftar_usaha as $key => $value)
      ...
      <div class="flex flex-col md:flex-row md:justify-start items-center rounded-md p-2 gap-2 shadow-[4px_5px_10px_0px_rgba(0,0,0,0.5)] ">
        ...
        <div
          ...
          <div onclick="{{ $value->id_status_pengajuan == 1 ? 'showKonfirmasi(' . $value->id_deskripsi_usaha . ')' : '' }}"
            ...
            Terkonfirmasi</div>
          <div onclick="{{ $value->id_status_pengajuan == 2 ? 'showTidakKonfirmasi(' . $value->id_deskripsi_usaha . ')' : '' }}"
        ...
        <div class="flex items-center justify-center gap-2 w-[12rem]">
          <div class="w-14 h-14 flex items-center justify-center">
            <a class="flex flex-col items-center hover:bg-neutral-200/70 duration-200 p-2"
              href="{{ $value->id_deskripsi_usaha > 3 ? asset('/storage' . $value->proposal) : $value->proposal }}">
              <img src="/icons/pdf.svg" alt="" class="w-12 h-12 ">
              <p class="font-medium font-roboto text-xs text-neutral-600 text-center">Lihat Proposal</p>
            </a>
          </div>
